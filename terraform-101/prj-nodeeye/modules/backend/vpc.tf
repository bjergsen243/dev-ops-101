...
resource "aws_vpc_endpoint" "ecr_api" {
  tags = {
    Name = "${lower(lookup(var.common, "customer"))}-${lower(terraform.workspace)}-vpce-ecr-api"
  }
  vpc_id            = aws_vpc.vpc.id
  service_name      = "com.amazonaws.${lookup(var.common, "${terraform.workspace}.region", var.common["default.region"])}.ecr.api"
...
resource "aws_vpc_endpoint" "ecr_dkr" {
  tags = {
    Name = "${lower(lookup(var.common, "customer"))}-${lower(terraform.workspace)}-vpce-ecr-dkr"
  }
  vpc_id            = aws_vpc.vpc.id
  service_name      = "com.amazonaws.${lookup(var.common, "${terraform.workspace}.region", var.common["default.region"])}.ecr.dkr"
  vpc_endpoint_type = "Interface"
